...
// resizes the total table
void CovidDB::resizeTable() {
    ...
    int tempInt = 2;
    // while loop to see if the number is prime
    while(isNotPrime) {
        // checks if temp int is less than half the table and the tablesize is not divisible by temp
        while (tempInt <= tableSize / 2 && tableSize % tempInt != 0) {
            tempInt++;
        }
        // if number is nonprime, continue on with loop
        if(tableSize % tempInt == 0) {
            isNotPrime = true;
            tableSize++;
        } else {
            // else break the loop
    ...
    // create a new empty vector to place into the array
    vector<DataEntry *> temp;
    for(int i = 0; i < tableSize; i++) {
